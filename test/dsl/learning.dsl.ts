...
export class LearningDSL extends AbstractDSL {
  ...
  getAll() {
    return this.req().get('/learning/courses').set(this.headers);
  }

  getAllByUser() {
    return this.req().get('/learning/me/courses').set(this.headers);
  }

  checkout(courseId: string) {
    return this.req()
      .post(`/learning/courses/${courseId}/checkout`)
      .set(this.headers);
  }
